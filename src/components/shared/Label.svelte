...
<label class="flex items-start justify-between flex-col md:flex-row">
	<div class="flex justify-start items-center">
		...
		{#if flag}
			<img src="https://flagcdn.com/24x18/{flag}.webp" alt="flag" class="ml-2" />
		{/if}
		<span class={`text-content-primary text-base font-inter font-medium ml-2 ${LabelClass}`}
			>{value}</span
		>
	</div>

	{#if tier}
		<div class="ps-8 pt-2 md:pt-0 md:ps-0">
			<TierLevel {tier} textClasses="!font-inter !text-sm !text-normal capitalize " />
		</div>
	{/if}
</label>
